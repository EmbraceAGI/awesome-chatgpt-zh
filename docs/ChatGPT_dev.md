...
|[Dakera](https://github.com/dakera-ai/dakera-mcp)|![GitHub Repo stars](https://badgen.net/github/stars/dakera-ai/dakera-mcp)|Open-source self-hosted MCP agent memory server with decay-weighted recall, HNSW vector search, and cross-agent knowledge sharing.|开源自托管 MCP Agent 记忆服务器，支持衰减加权召回、HNSW 向量搜索和跨 Agent 知识共享。|
